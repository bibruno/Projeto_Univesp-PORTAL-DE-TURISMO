...
def import_spots():
    ...
    with open(csv_path, encoding='utf-8') as csvfile:
        ...
        for row in reader:
            try:
                # Verificar se o ponto turístico já existe
                if TouristSpot.objects.filter(place_id=row['Place_ID']).exists():
                    print(f'Ponto turístico já existe: {row["Nome"]}')
                    continue
